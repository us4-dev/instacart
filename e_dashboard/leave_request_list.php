        ...
        <table class="table sakotable table-bordered table-striped dt-responsive">
          <thead>
            <tr>
			  ...
			  <th><?php echo $_data['leave_from']; ?></th>
			  <th><?php echo $_data['leave_to']; ?></th>
          ...
          <tbody>
            <?php
				$result = mysqli_query($link,"Select * from tbl_employee_leave_request r inner join tbl_add_employee e on e.eid = r.employee_id inner join tbl_add_member_type m on m.member_id = e.e_designation where r.branch_id = '" . (int)$_SESSION['objLogin']['branch_id'] . "' and r.employee_id = '" . (int)$_SESSION['objLogin']['eid'] . "' order by r.request_date ASC");
				while($row = mysqli_fetch_array($result)){?>
            <tr>
              <td><?php echo date('d/m/Y', strtotime($row['request_date'])); ?></td>
			  <td><?php echo $ams_helper->mySqlToDatePicker($row['from']); ?></td>
			  <td><?php echo $ams_helper->mySqlToDatePicker($row['to']); ?></td>
			  <td><label class="label label-success ams_label"><?php echo $ams_helper->daysBetween($row['from'], $row['to']);?></label></td>
			  <td><label class="label label-<?php if($row['request_status']=='Accepted'){echo 'success';} elseif($row['request_status']=='Rejected'){echo 'danger';}else{echo 'warning';}?> ams_label"><?php echo $row['request_status']; ?></label></td>
              <td><a class="btn btn-success ams_btn_special" data-toggle="tooltip" href="javascript:;" onClick="$('#nurse_view_<?php echo $row['leave_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a> <a class="btn btn-warning ams_btn_special" data-toggle="tooltip" href="<?php echo WEB_URL;?>e_dashboard/add_leave_request.php?id=<?php echo $row['leave_id']; ?>" data-original-title="<?php echo $_data['edit_text'];?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger ams_btn_special" data-toggle="tooltip" onClick="deleteLeaveData(<?php echo $row['leave_id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>"><i class="fa fa-trash-o"></i></a>
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="row">
                          <div class="col-xs-6"> <b><?php echo $_data['requested_date']; ?> :</b> <?php echo date('d/m/Y', strtotime($row['request_date'])); ?><br/>
						  	<b><?php echo $_data['leave_from']; ?> :</b> <?php echo $ams_helper->mySqlToDatePicker($row['from']); ?><br/>
...
function deleteLeaveData(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL; ?>e_dashboard/leave_request_list.php?id=' + Id;
	}
  }
