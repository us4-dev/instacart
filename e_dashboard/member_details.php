    ...
    <div class="box box-success">
      ...
      <div class="box-body">
        <table class="table sakotable table-bordered table-striped dt-responsive">
          ...
          <tbody>
            <?php
				$result = mysqli_query($link,"Select *,mt.member_type as mt_type from tbl_add_management_committee mc inner join tbl_add_member_type mt on mt.member_id = mc.member_type where mc.branch_id = '". (int)$_SESSION['objLogin']['branch_id'] . "' order by mc_id desc");
            ...
            <tr>
              ...
              <td><label class="label label-success ams_label"><?php echo $row['mt_type']; ?></label></td>
              <td><?php echo $row['mc_email']; ?></td>
              <td><?php echo $row['mc_contact']; ?></td>
              <td><?php echo $row['mc_pre_address']; ?></td>
              <td><a class="btn btn-success ams_btn_special" data-toggle="tooltip" href="javascript:;" onClick="$('#nurse_view_<?php echo $row['mc_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a>
