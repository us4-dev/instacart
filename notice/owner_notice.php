...
$form_url = WEB_URL . "notice/owner_notice.php";
$hval = 0;
...
if(isset($_POST['txtNoticeTitle'])){
	if($_POST['hdnSpid'] == '0'){
		...
		if(isset($_POST['ddlNoticeStatus']) && $_POST['ddlNoticeStatus']=='1'){
			mysqli_query($link,"UPDATE tbl_owner_notice_board set notice_status=0 WHERE branch_id=".$_SESSION['objLogin']['branch_id']);
		}
		$sql="INSERT INTO `tbl_owner_notice_board`(`notice_title`,`notice_description`,`notice_status`,`branch_id`,`created_date`) VALUES ('$_POST[txtNoticeTitle]','".$_POST['txtNoticeBoard']."','".$_POST['ddlNoticeStatus']."','".$_SESSION['objLogin']['branch_id']."','".$ams_helper->datepickerDateToMySqlDate($_POST['txtNoticeDate'])."')";	
		mysqli_query($link,$sql);
		mysqli_close($link);
		$url = WEB_URL . 'notice/owner_notice.php?m=add';
		header("Location: $url");
	} else {
		//unpublish other
		if(isset($_POST['ddlNoticeStatus']) && $_POST['ddlNoticeStatus']=='1'){
		...
		$sql_update="UPDATE `tbl_owner_notice_board` set notice_title = '$_POST[txtNoticeTitle]',notice_description = '".$_POST['txtNoticeBoard']."',notice_status = '".$_POST['ddlNoticeStatus']."',created_date = '".$ams_helper->datepickerDateToMySqlDate($_POST['txtNoticeDate'])."' where notice_id= '" . (int)$_POST['hdnSpid'] . "'";	
		mysqli_query($link,$sql_update);
		mysqli_close($link);
		$url = WEB_URL . 'notice/owner_notice.php?m=up';
		header("Location: $url");
	}
	$success = "block";
}
...
if(isset($_GET['notice_id']) && $_GET['notice_id'] != ''){
	$result_location = mysqli_query($link,"SELECT * FROM tbl_owner_notice_board where branch_id= '".(int)$_SESSION['objLogin']['branch_id']."' and notice_id=".(int)$_GET['notice_id']);
	if($row = mysqli_fetch_array($result_location)){
		...
		$form_url = WEB_URL . "notice/owner_notice.php?id=".$_GET['notice_id'];
		$hval = $row['notice_id'];
	}	
}
?>
...
<section class="content-header">
  <h1><?php echo $_data['text_13'];?></h1>
  <ol class="breadcrumb">
    <li><a href="<?php echo WEB_URL?>dashboard.php"><i class="fa fa-dashboard"></i> <?php echo $_data['home_breadcam'];?></a></li>
    <li class="active"><?php echo $_data['text_3bb'];?></li>
    <li class="active"><?php echo $_data['text_13'];?></li>
  </ol>
</section>
<!-- Main content -->
      ...
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
        <div class="box-body row">
          ...
          <div class="form-group col-md-12" align="right">
			...
			&nbsp;
            <input type="reset" onClick="javascript:window.location.href='<?php echo WEB_URL; ?>notice/owner_notice.php';" name="btnReset" id="btnReset" value="<?php echo $_data['reset'];?>" class="btn btn-warning"/>
          </div>
        </div>
        <input type="hidden" name="hdnSpid" value="<?php echo $hval; ?>"/>
      </form>
      <h4 style="text-align:center; color:red;"><?php echo $_data['text_555'];?></h4>
      <!-- /.box-body -->
<?php
$delinfo = 'none';
$addinfo = 'none';
...
if(isset($_GET['delid']) && $_GET['delid'] != '' && $_GET['delid'] > 0){
	$sqlx= "DELETE FROM `tbl_owner_notice_board` WHERE notice_id = ".$_GET['delid'];
          ...
          <div class="box box-success">
            ...
            <div class="box-body">
              <table class="table sakotable table-bordered table-striped dt-responsive">
                ...
                <tbody>
                  <?php
				$result = mysqli_query($link,"SELECT * FROM tbl_owner_notice_board where branch_id=".(int)$_SESSION['objLogin']['branch_id']);
				while($row = mysqli_fetch_array($result)){?>
                  <tr>
					...
					<?php } ?>
                    <td><a class="btn btn-warning ams_btn_special" data-toggle="tooltip" href="<?php echo WEB_URL;?>notice/owner_notice.php?notice_id=<?php echo $row['notice_id']; ?>" data-original-title="<?php echo $_data['edit_text'];?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger ams_btn_special" data-toggle="tooltip" onclick="deleteNotice(<?php echo $row['notice_id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>"><i class="fa fa-trash-o"></i></a></td>
                  </tr>
...
<script type="text/javascript">
  ...
  function deleteNotice(Id){
	...
	if(iAnswer){
		window.location = '<?php echo WEB_URL;?>notice/owner_notice.php?delid=' + Id;
	}
  }
  </script>
