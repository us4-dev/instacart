...
$form_url = WEB_URL . "o_dashboard/addcomplain.php";
$id="";
$hdnid="0";
...
if(isset($_POST['txtCTitle'])){
	...
	if(isset($_POST['hdn']) && $_POST['hdn'] == '0'){
		$sql = "INSERT INTO tbl_add_complain(c_title,c_description, c_date, c_month,c_year,c_userid,branch_id,complain_by,person_name,person_email,person_contact) values('$_POST[txtCTitle]','$_POST[txtCDescription]','$_POST[txtCDate]',$xmonth,$xyear,'".(int)$_SESSION['objLogin']['ownid']."','" . $_SESSION['objLogin']['branch_id'] . "','owner','" . $_SESSION['objLogin']['o_name'] . "','" . $_SESSION['objLogin']['o_email'] . "','" . $_SESSION['objLogin']['o_contact'] . "')";
		mysqli_query($link,$sql);
		mysqli_close($link);
		$url = WEB_URL . 'o_dashboard/complainlist.php?m=add';
		header("Location: $url");
	}
	else{
		...
		mysqli_query($link,$sql);
		$url = WEB_URL . 'o_dashboard/complainlist.php?m=up';
		header("Location: $url");
	}
	$success = "block";
}
...
if(isset($_GET['id']) && $_GET['id'] != ''){
	...
	while($row = mysqli_fetch_array($result)){
		...
		$form_url = WEB_URL . "o_dashboard/addcomplain.php?id=".$_GET['id'];
	}
}	
?>
      ...
      <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
		...
		<div class="box-footer">
          <div class="form-group pull-right">
            ...
            <a class="btn btn-warning" href="<?php echo WEB_URL; ?>o_dashboard/complainlist.php"><i class="fa fa-reply"></i> <?php echo $_data['back_text'];?></a> </div>
        </div>
