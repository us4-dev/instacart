    ...
    <div class="box box-success">
      ...
      <div class="box-body">
		...
		$result = mysqli_query($link,"Select *,fl.floor_no as fl_floor,u.unit_no as u_unit,orl.owner_id,w.o_name,w.image,m.month_name from tbl_add_fair ou inner join tbl_add_floor fl on fl.fid = ou.floor_no inner join tbl_add_unit u on u.uid = ou.unit_no inner join tbl_add_owner_unit_relation orl on orl.unit_id = u.uid inner join tbl_add_owner w on w.ownid = orl.owner_id inner join tbl_add_month_setup m on m.m_id = ou.month_id where ou.type='Owner' and ou.branch_id = " . (int)$_SESSION['objLogin']['branch_id'] . " order by ou.f_id desc");
            ...
            <td>
            <a class="btn btn-success ams_btn_special" data-toggle="tooltip" href="javascript:;" onclick="$('#nurse_view_<?php echo $row['f_id']; ?>').modal('show');" data-original-title="<?php echo $_data['view_text'];?>"><i class="fa fa-eye"></i></a> <a class="btn btn-warning ams_btn_special" data-toggle="tooltip" href="<?php echo WEB_URL;?>owner_utility/add_owner_utility.php?id=<?php echo $row['f_id']; ?>" data-original-title="<?php echo $_data['edit_text'];?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger ams_btn_special" data-toggle="tooltip" onclick="deleteOwnerUtility(<?php echo $row['f_id']; ?>);" href="javascript:;" data-original-title="<?php echo $_data['delete_text'];?>"><i class="fa fa-trash-o"></i></a>
				  ...
				  <div class="modal-body">
                    ...
                    <div class="row">
                      <div class="col-xs-6"> 
						...
						<b><?php echo $_data['add_new_form_field_text_17'];?> :</b> <?php echo $row['xyear'];?><br/>
                        <b><?php echo $_data['add_new_form_field_text_18'];?> : </b> <?php echo $ams_helper->currency($localization, $row['rent']); ?><br/>
